...
│
├── commands/wf/                     # 【已全量清空】22 个命令全部迁至 wf/skills/（Phase E 完成）
│   ├── do.md                        # /wf-do <描述>
...
│   ├── status.md                    # /wf-status [--auto-advance] （合并原 progress + next）
...
安装方式 (wf/bin/install.sh):
  commands/wf/  →  （已清空，全部迁至 $HOME/.claude/skills/wf-*/SKILL.md）
...
└──────┬───────┘
  ...
  │  "下一步"      → /wf-status --auto-advance  ◀── 智能路由 │
...
═══════════════════════════════════════════════════════════════════════════════════
  ...
  └──────────────────┘    └──────────────────┘    └────────┬─────────┘
    ...
    提示 /wf-status --auto-advance       Skill(对应工作流)      Skill(对应工作流)
                                               │                         │
                                               └────────────┬────────────┘
                                                            │
                                                            ▼
...
```
wf/
├── skills/                          # 【Phase E + P0 收敛】25 个 skill
...
├── bin/ templates/                  # 不动
│
commands/wf/                         # 【已删除】22 个命令全部迁至 wf/skills/（Phase E 完成）
...
- `validate_install` 哨兵检查 15 个关键 skill 存在（含 wf-do / wf-pause / wf-resume / wf-settings）

### 向后兼容

- `/wf-xxx` 入口行为不变：全部 22 个命令由 `wf/skills/wf-<name>/SKILL.md` 提供
- `commands/wf/` 目录已完全删除；历史 repo 意外残留的 command 文件由 install.sh 在卸载时清理
- 老版本用户升级时，旧 `commands/wf/*.md` 会被新 skill 同名覆盖并最终清理
- 107 个既有测试（spec 33 + change 56 + milestone 18）零回归

### Phase E 意义

1. **用户问"项目进度怎么样" → Claude 自动触发 `wf-status`**（P0 后合并自 wf-progress / wf-next）
2. **用户在 WF 项目写 git commit → Claude 自动加载 `wf-git-conventions`**（P0 后：非 WF 仓库不触发）
...
诊断（`wf-spicy-pudding.md`）识别 autonomous 长链路三处"信息已存在但被重新生成"的浪费，P1 做智能裁剪。**仅影响 autonomous 编排时的系统层决策，手动模式零行为差异。**

### 三项改动

| # | 优化 | 文件 | 配置开关 | 默认 | 节省估算 |
|---|---|---|---|---|---|
| 1 | autonomous 调 plan-phase 默认附加 `--skip-research` | `autonomous.md` § 2.3 | `workflow.research_in_autonomous` | `false` | ~15% |
| 2 | autonomous Phase 1 冷启动跳过 discuss | `autonomous.md` § 2.2 | `workflow.phase1_cold_start_window_sec` | `3600` | 5-8% |
| 3 | verify-work smoke 复用近期 VERIFICATION.md | `verify-work.md` § 2.0 | `workflow.verification_reuse_window_sec` | `600` | ~10% |

### 冷启动与复用判定的"全部条件"

**Phase 1 冷启动跳过 discuss**：
- `phase === 1`
- `.planning/PROJECT.md` 存在且 mtime < 窗口（默认 1 小时）
- `.planning/phases/01-*/CONTEXT.md` 不存在
- 未传 `--interactive`

命中则生成占位 CONTEXT.md（`auto_generated: true`，指向 PROJECT.md），保持 hook 状态机一致。

**Smoke 复用 VERIFICATION.md**：
- 文件存在
- mtime < 窗口（默认 10 分钟）
- 无 `FAIL`
- `DATA-FLOWING` 标记为 PASS

命中则跳过 `npm run build` + `npm test`；`curl` 启动检查永远跑。

### 对用户的可见变化

- 新项目 `/wf-new-project --auto` → `/wf-autonomous` 的完整链路代币消耗 **-20 ~ -30%**
- 老项目 `/wf-autonomous --from N` 不受影响（PROJECT.md 窗口不命中）
- 手动 `/wf-plan-phase N` / `/wf-verify-work --smoke` 不受影响
- config.workflow 新增 3 个开关可覆盖默认行为
