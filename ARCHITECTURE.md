...
│
├── commands/wf/                     # 4 个纯 command（其余 18 个命令已迁至 wf/skills/，Phase E 后）
│   ├── do.md                        # /wf-do <描述>
...
安装方式 (wf/bin/install.sh):
  commands/wf/  →  $HOME/.claude/commands/wf/    (4 个纯 command：do/pause/resume/settings)
...
**共识**：22 个 skill 中 **12 个开放触发、8 个 `disable-model-invocation: true`、2 个 `user-invocable: false`**。`wf-code-review` 是唯一使用 `context: fork` 的 skill。

### 目录结构（Phase E 后）

```
wf/
├── skills/                          # 【新增】22 个 skill
...
├── bin/ templates/                  # 不动
│
commands/wf/                         # 【瘦身】4 个纯 command：do / pause / resume / settings
                                     # 其余 18 个已迁至 wf/skills/（Phase E 完成）
...
- `validate_install` 哨兵检查 15 个关键 skill 存在

### 向后兼容

- `/wf-xxx` 入口行为不变：18 个迁入 skill 的命令由 SKILL.md 提供；4 个纯 command（do / pause / resume / settings）保留为 `commands/wf/*.md`
- 旧的 14 个 command 文件在 Phase E 完成后已删除（skill 同名取代，避免维护两套）
