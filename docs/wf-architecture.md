...
| `wf-status --auto-advance` | 自动检测并路由到下一步 | 无参数（或 `--auto-advance`） |
...
| `wf-status` | 进度仪表板 + 智能路由（合并自 wf-progress + wf-next） | `[--auto-advance]` |
...
| **状态检查** | `/wf-status`（查询）或 `/wf-status --auto-advance`（自动路由） |
...
8. **薄路由层：** `/wf-status --auto-advance` 和 `/wf-do` 是纯路由器，调用已有 Skill 不重复实现逻辑。
