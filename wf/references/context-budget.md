...
阈值定义和门禁行为见 [gates.md](./gates.md#1-上下文预算门禁-context-budget-gate)。

`wf-context-monitor` hook 在每次工具调用后检查 context 使用率：
- **WARNING (剩余 <= 30%):** Hook 注入警告，建议确保 CONTINUATION.md 检查点已写入
- **CRITICAL (剩余 <= 15%):** Hook 注入严重警告，确认检查点已就绪，auto-compact 即将触发

**防抖机制:** 同级别警告至少间隔 60 秒触发一次。级别升级（WARNING -> CRITICAL）立即触发，不受防抖限制。
...
- **Smoke-only 验证** 自主模式默认使用 `--smoke` 跳过对话式 UAT
