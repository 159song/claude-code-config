...
4. 显示恢复横幅（ui-brand 标准横幅: 恢复位置、剩余阶段、上次结果）
5. 跳转到对应阶段的对应步骤继续执行（例如 step=execute 则跳过 discuss 和 plan）

如果不存在 CONTINUATION.md，显示正常执行横幅（ui-brand 标准横幅: 执行范围、阶段列表）。
...
显示阶段完成横幅（ui-brand 进度格式: 任务数、验证状态）。
...
显示完成汇总（ui-brand 标准横幅: 各阶段状态、总任务数、总验证数）。
...
显示自动归档横幅，通过 `Skill(complete-milestone, { version: {{milestone_version}} })` 执行。

**如果 `auto_archive_on_complete === false` 或字段不存在：**

提示: `/wf-verify-work` 进行最终验收，`/wf-complete-milestone` 归档里程碑。
